...
func init() {
	flag.StringVar(&Servers, "servers", "user:passwd@tcp(127.0.0.1:3306)/?tls=false", "Set mysql servers")
}
...
var (
	localhost        = "zabbix:08IMrvup@tcp(127.0.0.1:3306)/?tls=false"
	lastT            time.Time
	initDone         = false
...
func (m *Mysql) Gather() error {
	...
	if len(m.Servers) == 0 {
		...
		return m.gatherServer(localhost)
	}
...
func (m *Mysql) gatherServer(serv string) error {
	...
	if m.GatherProcessList {
		// ignore error
		// mysql 5.0 have no processList table
		m.GatherProcessListStatuses(db, serv)
	}
...
// MySQL environment.
func (m *Mysql) gatherGlobalVariables(db *sql.DB, serv string) error {
	// run query
...
func (m *Mysql) gatherSlaveStatuses(db *sql.DB, serv string) error {
	// run query
...
func (m *Mysql) gatherBinaryLogs(db *sql.DB, serv string) error {
	// run query
...
func (m *Mysql) gatherGlobalStatuses(db *sql.DB, serv string) error {
	...
	if m.GatherUserStatistics {
		...
		if err != nil {
			...
			for conn_rows.Next() {
				...
				merices("user.stats", fields, tags)
			}
		}
	}
...
func (m *Mysql) GatherProcessListStatuses(db *sql.DB, serv string) error {
	// run query
	...
	merices("info.schema", fields, tags)
	return nil
}

// GatherUserStatistics can be used to collect metrics on each running command
// and its state with its running count
func (m *Mysql) GatherUserStatisticsStatuses(db *sql.DB, serv string) error {
	// run query
...
func (m *Mysql) gatherPerfTableIOWaits(db *sql.DB, serv string) error {
	...
	for rows.Next() {
		...
		merices("perf.schema", fields, tags)
	}
	return nil
}
...
func (m *Mysql) gatherPerfIndexIOWaits(db *sql.DB, serv string) error {
...
func (m *Mysql) gatherInfoSchemaAutoIncStatuses(db *sql.DB, serv string) error {
...
func (m *Mysql) gatherInnoDBMetrics(db *sql.DB, serv string) error {
	// run query
...
func (m *Mysql) gatherPerfTableLockWaits(db *sql.DB, serv string) error {
	...
	for rows.Next() {
		...
		merices("perf.schema", sqlLWFields, sqlLWTags)
		...
		merices("perf.schema", externalLWFields, externalLWTags)
		...
		merices("perf.schema", sqlLWSecTotalFields, sqlLWSecTotalTags)
		...
		merices("perf.schema", externalLWSecTotalFields, externalLWSecTotalTags)
	}
	return nil
}

// gatherPerfEventWaits can be used to get total time and number of event waits
func (m *Mysql) gatherPerfEventWaits(db *sql.DB, serv string) error {
...
func (m *Mysql) gatherPerfFileEventsStatuses(db *sql.DB, serv string) error {
	...
	for rows.Next() {
		...
		merices("perf.schema", fields, miscTags)
		...
		merices("perf.schema", fields, readTags)
		...
		merices("perf.schema", fields, writeTags)
...
func (m *Mysql) gatherPerfEventsStatements(db *sql.DB, serv string) error {
	query := fmt.Sprintf(
		perfEventsStatementsQuery,
...
func (m *Mysql) gatherTableSchema(db *sql.DB, serv string) error {
	var dbList []string
	servtag := getDSNTag(serv)
	...
	for _, database := range dbList {
		...
		for rows.Next() {
			...
			merices(newNamespace("info.schema", "table_version"),
...
func merices(name string, fields map[string]interface{}, tags map[string]string) {
	...
	for k, v := range fields {
		...
		fv, floatok := v.(float64)
		iv, intok := v.(int)
		i64v, int64ok := v.(int64)
		if floatok {
			value = fv
		} else if intok {
			value = iv
		...
		m.Value = value
		m.Tags = tags
		delete(tags, "gatherGlobalStatuses")
		delete(tags, "version_compile_machine")
		delete(tags, "version_compile_os")
		delete(tags, "protocol_version")
		delete(tags, "version")
		m.Timestamp = ts
		ms = append(ms, m)
	}
...
func main() {
	...
	var m Mysql
	m.IntervalSlow = "30"
	m.Servers = strings.Split(Servers, ",")
